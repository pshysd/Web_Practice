	...
	<!--
		* JSP를 이루는 구성인자
		...
		3. JSP 액션 태그 : XML 기술을 이용해서 기존의 jsp 문법을 확장하는 기술을 제공하는 태그
							(XML 태그는 반드시 시작 태그와 종료 태그가 꼭 한 세트로 있어야만 함)
			- 표준 액션 태그 (Standard Action Tag)
				: JSP 페이지에서 바로 사용 가능한 태그 (별도의 라이브러리 연동이 필요 없음)
					표준 액션 태그는 모든 태그명 앞에 jsp: 이라는 접두어가 붙음

			- 커스텀 액션 태그 (Custom Action Tag)
				: JSP 페이지에서 바로 사용 불가능한 태그 (별도의 라이브러리 연동이 필요함 -> taglib 지시어 필요)
					커스텀 액션 태그는 모든 태그명 앞에 jsp: 이외의 다른 접두어가 붙음 (내가 지정한 접두어, 종류는 다양함)
					대표적인 라이브러리는 JSTL
	  -->
	  ...
	  <p>
		JSP 페이지에서 별도의 라이브러리 연동이 없이 곧바로 사용 가능한 태그들 <br>
