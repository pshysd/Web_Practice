...
<body>
			...
			<div class="modal-content">
				...
				<div class="modal-footer">
					<input type="checkbox" id="mainPop" name="mainPop"> <label
						for="mainPop">24시간 동안 보지 않기</label>
				</div>
	...
	<script>
		...
		// 메인페이지(index.jsp)를 응답해주는 서블릿이 존재하지 않기 때문에
		// 서버단에서 쿠키를 생성해서 첨부할 수가 없음 -> 프론트에서 자바스크립트로 쿠키를 만들어줘야 함
		
		// 24시간동안 보지 않기 체크박스가 클릭되는 순간 mainPop이라는 키값을 가진 쿠키 생성
		$('#mainPop').click(function() {
			
			// 광고창 닫기
			$('#adModal').modal('hide');
			
			// 쿠키 생성 -> mainPop의 여부만 검사할 것이기 때문에 쿠키의 value는 형태에 맞게 하드코딩
			
			// 현재 시간 불러오기 -> 하루 더해주기 (setDate() 메소드 활용)
			let today = new Date();
			today.setDate(today.getDate() + 1); // 날짜를 세팅해주는 메소드
			
			// 자바스크립트에서 쿠키를 생성하려면
			// document 객체에서 제공하는 cookie 속성에 접근해야 한다
			// 단순히 문자열 대입만으로 쿠키가 생성될 수 있음
			// 단, 형식에 맞는 문자열을 대입해줘야 함 -> 'key = value; expires=만료시간'
			// (이 때, 만료시간은 UTC 기준으로 잡아줘야 함 - 세계표준시)
		...
		if(document.cookie.includes('mainPop=n')){
			...
			// mainPop이라는 쿠키가 없다. == 광고창이 보여도 상관없다.
			
			// 모달창을 열어주는 부트스트랩 메소드 : .modal('show');
			$('#adModal').modal('show');
			}
		})
	</script>
